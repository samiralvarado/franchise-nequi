...
class UpdateProductNameUseCaseTest {
    ...
    void shouldUpdateProductName() {
        Product product = Product.builder().id("product-id").name(ProductName.of("Old product")).stock(ProductStock.of(10)).build();
        ...
        StepVerifier.create(useCase.updateProductName("franchise-id", "branch-id", "product-id", "New product"))
                .expectNextMatches(saved -> saved.getBranches().getFirst().getProducts().getFirst().getName().getValue().equals("New product"))
                .verifyComplete();
    }

    @Test
    void shouldFailWhenProductDoesNotExist() {
        Branch branch = Branch.builder().id("branch-id").name(BranchName.of("Main Branch")).products(List.of()).build();
        Franchise franchise = Franchise.builder().id("franchise-id").branches(List.of(branch)).build();

        when(franchiseRepository.findById("franchise-id")).thenReturn(Mono.just(franchise));

        StepVerifier.create(useCase.updateProductName("franchise-id", "branch-id", "missing-id", "New product"))
